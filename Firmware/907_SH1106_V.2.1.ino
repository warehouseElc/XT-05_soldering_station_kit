// Soldering Iron 907 mod by Warehouse_elc
// mods : OLED_SH1106 DISP I2C
// Firmware Version 2.1 
// Board Version 4.1
// Origin : v.2 https://create.arduino.cc/projecthub/sfrwmaker/soldering-iron-controller-for-hakko-907-v-2-fc75d7
...
const uint16_t temp_minC = 200;     // Minimum temperature in degrees of celsius
const uint16_t temp_maxC = 480;     // Maximum temperature in degrees of celsius
...
private:
  ...
  const uint16_t ambient_temp = 0; // Ambient temperatire in the internal units
...
class DSPL: protected U8X8_SH1106_128X64_NONAME_HW_I2C {
public:
  DSPL(void) :
...
void DSPL::tSet(uint16_t t, bool celsius) {
  ...
  if (!celsius)
    ...
    U8X8::setCursor(2,0);
    sprintf(buff, "%3d%c%c", t,'\x27' , units);
    U8X8::print(buff);                                     // draw setpoint temperature
...
private:
  ...
  const byte max_power = 200;       // maximum power to the iron (220)
  const byte max_fixed_power = 150;      // Maximum power in fiexed power mode
...
void workSCREEN::show(void) {
  ...
  if ((abs(temp_set - temp) < 5) && (pIron->tempDispersion() <= 20)) {
    if (!ready) {
      idle_power.put(ap);
      pBz->shortBeep();
      ready = true;
...
void calibSCREEN::show(void) {
  ...
  if (!pIron->isOn())
    p = 0;
  pD->percent(p);
  if (tune && (abs(temp_set - temp) < 5) && (pIron->tempDispersion() <= 20)) {
    if (!ready) {
      pBz->shortBeep();
      pD->msgReady();
      ready = true;
